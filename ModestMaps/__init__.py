...
def calculateMapExtent(provider, width, height, *args):
    ...
    """
    coordinates = [provider.locationCoordinate(a) for a in args]
...
class TileRequest:
    ...
    def load(self, lock, verbose, attempt=1):
        ...
        try:
            ...
            for (scheme, netloc, path, params, query, fragment) in map(urlparse, urls):
                ...
                if str(response.status).startswith('2'):
                    imgs.append(PIL.Image.open(io.StringIO(response.read())).convert('RGBA'))
